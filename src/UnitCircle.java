...
class UnitCirclePanel extends JPanel implements MouseMotionListener {
	...
	g.drawString("B("+pB.x+", "+pB.y+")", (int)pB.x+10, (int)pB.y-30);
	...
	g.drawString("C("+pC.x+", "+pC.y+")", (int)pC.x+10, (int)pC.y-30);

	// point A
	g.setColor(reColor);
        g.fill(new Ellipse2D.Double(pA.x-10, pA.y-10, 20, 20));
	g.drawString("A("+x+", "+y+")", (int)pA.x+10, (int)pA.y-30);
	
	// Hyp, C and B midpoint
	g.setColor(reColor);
	//g.fill(new Ellipse2D.Double((((cx-10)+(centerX-10))/2),
	//			    (((cy-10)+(centerY-10))/2), 10, 10));
	
	g.drawString("Hyp", (int)(((cx-10)+(centerX-10))/2),
		                 (int)(((cy-10)+(centerY-10))/2));
	...
	// Screen Text // BLACK COLOR
        ...
        g.drawString("Angle θ: "+Math.toDegrees(Math.acos(x)), 76, 32);	
     }
